...
a. Fixing the Application Pool
b. Updated the application pool configuration on VMSS2 to ensure it started automatically:
...
d. Automated the fix for all instances in VMSS2 using an Azure Custom Script Extension. The script ensured consistent configuration across scale set instances:

# Custom Script for IIS Configuration
appcmd set apppool /apppool.name:"DefaultAppPool" /autoStart:true
iisreset


e. Updating Health Probe Configuration
f. Changed the health probe path to point to a more reliable endpoint (/default.aspx) instead of the custom /healthcheck endpoint:
